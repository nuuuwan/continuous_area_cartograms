...
class MultiStageCAC:
    ...
    def run(self, dir_path: str=None):
        ...
        for i in range(1, len(self) + 1):
            dir_path_stage_images = os.path.join(dir_path, f'stage_{i}', 'images')
            for file_name in os.listdir(dir_path_stage_images):
                image_path_list.append(os.path.join(dir_path_stage_images, file_name))
            
        animated_gif_path = os.path.join(dir_path, 'animated.gif')
        AnimatedGIF(animated_gif_path).write_from_image_path_list(image_path_list)